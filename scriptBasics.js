...
let algoDesc = document.getElementById('algoDesc');
let td = document.getElementsByTagName('td')
let pv = document.getElementById('pathVisited')
let ListOfNodes = [];
let nodes = [];
let visiteds = [];
let vis = [];
...
function algoVis(algo) {
    ...
    if (algo.includes('Dijkstra')) {
        ...
        let node = bredthFirstSearch();
        vis = node[0];
        path = node[1];
    } if (algo.includes('Best first search')) {
        let node = bestFirstSearch();
        vis = node[0];
        path = node[1];
    } if (algo.includes('A *')) {
        let node = AStar();
